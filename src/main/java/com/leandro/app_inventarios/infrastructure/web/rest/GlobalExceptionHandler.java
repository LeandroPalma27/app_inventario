...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleBadRequest(Exception ex, HttpServletRequest request) {
        return buildError(HttpStatus.BAD_REQUEST, "BAD_REQUEST", "Invalid request", ex, request);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleNoHandlerFound(NoResourceFoundException ex, HttpServletRequest request) {
        return buildError(HttpStatus.NOT_FOUND, "ENDPOINT_NOT_FOUND", ex.getMessage(), ex, request);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> customHandleNotFound(NotFoundException ex, HttpServletRequest request) {
        return ApiResponseFactory.buildError(request.getRequestURI(), HttpStatus.NOT_FOUND, ex.getCode(), ex.getMessage());
    }

    @ExceptionHandler({NoSuchElementException.class})
    public ResponseEntity<ApiResponse<Void>> handleNoSuchElement(NoSuchElementException ex, HttpServletRequest request) {
        return buildError(HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", ex.getMessage(), ex, request);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleGeneric(Exception ex, HttpServletRequest request) {
        return buildError(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "Unexpected error occurred", ex, request);
    }

    // -------------------------
    // Helper común
    // -------------------------
    private ResponseEntity<ApiResponse<Void>> buildError(HttpStatus status, String code, String message, Exception ex, HttpServletRequest request) {
        ApiError error = new ApiError(code, message);
        return ResponseEntity.status(status).body(ApiResponse.error(status.is2xxSuccessful(), error, request.getRequestURI(), status.value()));
    }
}
